...
export async function initCommand(options: InitOptions) {
  ...
  try {
    ...
    await fs.copy(
      path.join(packageRoot, 'PRPs', 'templates'),
      path.join(cwd, 'PRPs', 'templates')
    );
...
`
    ...
    await fs.copy(
      path.join(packageRoot, 'TECH_STACK.md'),
      path.join(cwd, 'TECH_STACK.md')
    );

    // Copy project.md.template
    spinner.text = 'Creating project.md template...';
    await fs.copy(
      path.join(packageRoot, 'project.md.template'),
      path.join(cwd, 'project.md.template')
    );
