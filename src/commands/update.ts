...
export async function updateCommand(options: UpdateOptions) {
  ...
  for (const rel of candidates) {
    if (packageFiles.includes(rel)) continue;
    const projectPath = path.join(cwd, rel);
  ...
  const printList = (label: string, color: (s: string) => string, files: string[]) => {
    if (files.length === 0) return;
    console.log(color(label));
    for (const f of files) console.log(color(`  ${f}`));
    console.log();
  };
  ...
  printList(`! ${skippedModified.length} locally modified — skipped (use --force to overwrite)`,
    chalk.red, skippedModified);
  printList(`! ${skippedObsolete.length} obsolete but locally modified — kept (use --force to remove)`,
    chalk.red, skippedObsolete);
  ...
  try {
    ...
    applySpinner.succeed(chalk.green(
      `✅ Updated: ${added.length} added, ${updated.length} updated, ${removed.length} removed`
    ));
