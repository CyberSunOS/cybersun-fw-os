...
export async function collectFrameworkFiles(root: string): Promise<string[]> {
  const files: string[] = [];
...
export async function buildManifest(root: string, version: string): Promise<Manifest> {
  const manifest: Manifest = { seedfwVersion: version, files: {} };
