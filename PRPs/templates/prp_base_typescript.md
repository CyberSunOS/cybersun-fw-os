...
> Commands below are the TS/web example — adapt to the project's stack (run its own gates as defined in package.json / Makefile). If a gate doesn't exist for the confirmed stack, substitute the closest equivalent and record the substitution. Level 0 (build) may never be skipped — only adapted.
