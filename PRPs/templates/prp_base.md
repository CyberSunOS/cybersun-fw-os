...
> Commands below are examples — adapt to the project's stack (run its own gates as defined in package.json / Makefile / pyproject.toml). If a gate doesn't exist for the confirmed stack, substitute the closest equivalent and record the substitution. Level 0 (build) may never be skipped — only adapted.
...
# Expected: Zero errors. Nothing else runs until Level 0 passes.
```
...
### Level 3: Functional Validation (System)
...
### Additional Validation (Optional — Creative & Domain-Specific)
