...
> Commands below are examples — adapt to the project's stack (run its own gates). If a gate doesn't exist for the confirmed stack, substitute the closest equivalent and record the substitution. Level 0 (build) may never be skipped — only adapted.
